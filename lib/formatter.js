function format(paramValues, prefix) {
	...
	for (let op in params) {
		if (params.hasOwnProperty(op)) {
			switch (op) {
				case 'rangeStart':
					paramObj.rangeStart = paramValues.rangeStart;
					break;

				case 'rangeEnd':
					paramObj.rangeEnd = paramValues.rangeEnd;
					break;
...
function flatten(obj) {
	...
	function makeFlat(obj, path) {
		...
		if (keys.length) {
			keys.forEach((key) => makeFlat(obj[key], (path ? path + '.' : path) + key));
		} else {
			flatObj[path] = obj;
		}
	}
